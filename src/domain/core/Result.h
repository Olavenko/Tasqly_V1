...
// [[nodiscard]] inline Result<void> ok(); // (deprecated placeholder) → use specialization below
...
// -------------------------------
// Allows representing operations that succeed/fail without a value payload.
// Example: Result<void> remove(...);
...
public:
  ...
  static Result ok() noexcept { return Result(std::in_place_index<0>); }

  // ➕ Constructs a failed void result
  static Result err(Error e) noexcept { return Result(std::in_place_index<1>, std::move(e)); }

  // ✅ State queries
  [[nodiscard]] bool isOk() const noexcept { return std::holds_alternative<std::monostate>(_data); }
  [[nodiscard]] bool isErr() const noexcept { return std::holds_alternative<Error>(_data); }
  [[nodiscard]] explicit operator bool() const noexcept { return isOk(); }
  ...
  template<typename VO, typename VE>
  [[nodiscard]] auto match(VO&& onOk, VE&& onErr) const
      -> std::common_type_t<std::invoke_result_t<VO>, std::invoke_result_t<VE, const Error&>>
  {
    using ROk = std::invoke_result_t<VO>;
    using RErr = std::invoke_result_t<VE, const Error&>;
    static_assert(std::is_same_v<ROk, RErr>, "match branches must return the same type");
    if (isOk())
      return std::invoke(std::forward<VO>(onOk));
    else
      return std::invoke(std::forward<VE>(onErr), std::get<1>(_data));
  }
...
private:
  ...
  explicit Result(std::in_place_index_t<0>)
      : _data(std::in_place_index<0>, std::monostate{})
  {}
  explicit Result(std::in_place_index_t<1>, Error e)
      : _data(std::in_place_index<1>, std::move(e))
  {}

  std::variant<std::monostate, Error> _data;
};

// 🔧 Free helpers for void
...
[[nodiscard]] inline Result<void> err(const Error& e)
{
  return Result<void>::err(e);
}

[[nodiscard]] inline Result<void> err(Error&& e)
{
