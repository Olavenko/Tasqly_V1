...
def extract_title_and_status(file: Path):
    """Extract ADR ID, title + status from file content"""
    title = file.stem
    adr_id = file.stem.split("-")[0]
    status = "Draft"
    try:
        with file.open("r", encoding="utf-8") as f:
            for line in f:
                # ADR Title format: "# ADR 000X: Title"
                if line.strip().startswith("# ADR"):
                    match = re.match(r"# ADR\s+(\d+):\s*(.+)", line.strip())
                    if match:
...
def collect_adrs():
    ...
    for f in sorted(ADR_DIR.glob("*.md")):
        ...
        if f.name[0:4].isdigit():
            adr_id, title, status = extract_title_and_status(f)
            ...
            groups[root_group].append([adr_id, title, status, last_modified,
                                       f"[{adr_id}]({f.name})"])
    groups[root_group].sort(key=lambda r: int(r[0]))
        ...
        for slice_dir in sorted(phase_dir.glob("slice*")):
            ...
            for f in sorted(slice_dir.glob("*.md")):
                ...
                groups[group_name].append([adr_id, title, status, last_modified,
                                           f"[{adr_id}]({relpath.as_posix()})"])
            groups[group_name].sort(key=lambda r: int(r[0]))
    return groups
...
def main():
    ...
    # root ADRs (if any)
    if groups.get("root"):
        count = len(groups["root"])
        sections.append(f"### Root ADRs ({count} ADRs)\n")
    ...
    for group, rows in groups.items():
        if group == "root":
            continue
        count = len(rows)
        sections.append(f"### {group} ({count} ADRs)\n")
    ...
    legend = [
        ...
        "",
        f"*Index generated on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}*"
    ]
